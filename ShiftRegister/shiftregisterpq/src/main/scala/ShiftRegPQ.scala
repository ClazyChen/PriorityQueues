...
object EntryBlock {
    def init_block(addressWidth : Int,priorityWidth : Int) : EntryBlock {
        val entry_init = new EntryBlock(addressWidth,priorityWidth);
        entry_init.address := 0.U(addressWidth.W);  // 设置默认情况下的entry buffer地址为0
        entry_init.priority := 0.U(priorityWidth.W); // 设置默认情况下的entry priority为0
        entry_init
    }
}
...
class PriorityQueueBlock(addressWidth: Int,priorityWidth : Int,depth : Int) extends Module {
    ...
    .elsewhen (io.read && !io.write) {
        ...
        regs(depth - 1).io.entry_holder := EntryBlock.init_block(addressWidth, priorityWidth); // block index为depth - 1处添加一个默认entry
    }
    .otherwise(
        // empty 
    )
...
object Main extends App {
    println("running app");
    (new ChiselStage).emitVerilog(new PriorityQueueBlock(6,10,8));
}