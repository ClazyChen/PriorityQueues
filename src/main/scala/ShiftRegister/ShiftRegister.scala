...
class ShiftRegisterBlock(priorityWidth: Int, flowWidth: Int) extends Module {
  ...
  val io = IO(new Bundle{
    ...
    val deq_Right_Entry = Output(new PriorityEntry(priorityWidth, flowWidth))         // 出队模式下，本模块的出队输出（用于传递给右侧模块）
...
class PriorityQueue(val priorityWidth: Int, val flowWidth: Int, val depth: Int) extends Module {
  ...
  // 最左侧没有左侧邻接模块，故将其 leftEntry 置为默认值（0）
  ...
  for(i <- 0 until depth-1) {
    modules(i).deq_Left_Entry := modules(i+1).stored
  }
}
