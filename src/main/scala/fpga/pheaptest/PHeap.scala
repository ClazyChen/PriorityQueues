...
class PHeap extends Module with PriorityQueueTrait {
  ...
  val left_levels: Seq[LeftMem] = Seq.tabulate(count_of_levels) { idx =>
    val level = idx + 1
    val data_width = 1 << idx
    val node_width = WireDefault(BNode.default(level)).asUInt.getWidth
    val heap_level  = Module(new LeftMem(data_width, node_width))
    heap_level
  }
  val right_levels: Seq[RightMem] = Seq.tabulate(count_of_levels) { idx =>
    val level = idx + 1
    val data_width = 1 << idx
    val node_width = WireDefault(BNode.default(level)).asUInt.getWidth
    val heap_level  = Module(new RightMem(data_width, node_width))
    heap_level
  }
