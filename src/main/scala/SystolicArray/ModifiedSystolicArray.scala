...
class ShiftRegisterBlock(priorityWidth: Int, flowWidth: Int) extends Module {
  val io = IO(new Bundle{
    ...
    val enqueue_Right_Entry = Input(new PriorityEntry(priorityWidth, flowWidth))   // 入队模式下，从右侧相邻模块接收移位数据及其标志
    val enqueue_Left_Entry  = Output(new PriorityEntry(priorityWidth, flowWidth))         // 入队模式下，将本模块需要右移的与元素通过此端口移位给左侧模块
...
class ModifiedSystolicArrayBlock(val priorityWidth: Int, val flowWidth: Int, val depth: Int) extends Module {
  ...
  shiftblocks(0).enqueue_Right_Entry  := PriorityEntry.default(priorityWidth, flowWidth)
  ...
  // 默认情况下，不执行任何操作
  for(i <- 0 until depth) {
    shiftblocks(i).new_Entry.bits := PriorityEntry.default(priorityWidth, flowWidth)
    shiftblocks(i).new_Entry.valid := false.B
  ...
  when(io.block_enqueue_Signal) {
    when(entry_counter < depth.U - 1.U) {
      ...
      for(i <- 0 until depth) {
        ...
        shiftblocks(i).new_Entry.valid := io.block_enqueue_Signal
